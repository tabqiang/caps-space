//ref：https://github.com/vitejs/vite/blob/main/scripts/releaseUtils.ts
...
export function run<EO extends ExecaOptions>(
    ...
    args: string[],
    opts?: EO,
): ResultPromise<EO & (keyof EO extends 'stdio' ? object : { stdio: 'inherit' })> {
...
export async function updateTemplateVersions(): Promise<void> {
    const vitePkgJson = JSON.parse(await fs.readFile('packages/vite/package.json', 'utf-8'));
    const viteVersion = vitePkgJson.version;
    if (/beta|alpha|rc/.test(viteVersion)) return;

    const dir = 'packages/create-vite';
    const templates = (await fs.readdir(dir)).filter(dir => dir.startsWith('template-'));
